...
class TestBotContextDependency:
    ...
    async def test_bot_not_found_returns_404(self, client) -> None:
        ...
        with patch("stonks_trading.domains.trading.repositories.BotInstanceRepository.get") as mock_get:
    ...
    async def test_bot_exists_returns_context(self, client) -> None:
        ...
        with patch("stonks_trading.domains.trading.repositories.BotInstanceRepository.get") as mock_get:
            with patch("stonks_trading.domains.trading.repositories.BotStateRepository.load") as mock_load:
...
class TestBotScopedRoutes:
    ...
    async def test_list_bot_trades_empty(self, client) -> None:
        ...
        mock_instance.status = "running"
        with patch("stonks_trading.domains.trading.repositories.BotInstanceRepository.get") as mock_get:
            with patch("stonks_trading.domains.trading.repositories.list_trades_by_bot") as mock_list:
                mock_get.return_value = mock_instance
    ...
    async def test_list_bot_trades_with_symbol_filter(self, client) -> None:
        ...
        with patch("stonks_trading.domains.trading.repositories.BotInstanceRepository.get") as mock_get:
            with patch("stonks_trading.domains.trading.repositories.list_trades_by_bot") as mock_list:
                mock_get.return_value = mock_instance
                mock_list.return_value = []
                response = await client.get("/api/v1/bots/neat_swing/test-bot/trades?symbol=BTC_USD&limit=50")
                assert response.status_code == 200
    ...
    async def test_list_bot_positions_empty(self, client) -> None:
        ...
        mock_instance.status = "running"
        with patch("stonks_trading.domains.trading.repositories.BotInstanceRepository.get") as mock_get:
            with patch("stonks_trading.domains.trading.repositories.list_positions_by_bot") as mock_list:
                mock_get.return_value = mock_instance
                mock_list.return_value = []
