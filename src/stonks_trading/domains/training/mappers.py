...
class SchedulerJobMapper:
    """Maps scheduled job data to API DTOs."""

    @staticmethod
    def to_response(job_data: dict[str, Any]) -> SchedulerJobResponse:
        """Convert scheduler job data to API response DTO."""
        return SchedulerJobResponse(
            job_id=job_data["job_id"],
            bot_type=job_data["bot_type"],
    ...
    @staticmethod
    def to_response_list(jobs_data: list[dict[str, Any]]) -> list[SchedulerJobResponse]:
        """Convert list of job data to response DTOs."""
        return [SchedulerJobMapper.to_response(j) for j in jobs_data]

    @staticmethod
    def to_list_response(jobs_data: list[dict[str, Any]]) -> SchedulerJobListResponse:
        """Convert job data to list response DTO."""
        return SchedulerJobListResponse(
            jobs=SchedulerJobMapper.to_response_list(jobs_data),
            total=len(jobs_data),
        )


class TriggerRetrainingMapper:
    """Maps trigger retraining results to API DTOs."""
    ...
    def to_response(
        ...
        """Convert retraining results to API response DTO."""
